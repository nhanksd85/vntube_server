...
{
    public class vtv1AdaptiveController : ApiController
    {
        public ResResult Get(string id0 = "", string id1 = "", string id2 = "", string id3 = "", string id4 = "")
        {
            string link = getLinkFromTV101("http://tv.101vn.com/ok/vtv/vtv13.php");
        ...
        private string getLinkFromTV101(string original)
        {
            ...
            if (firstResult != "" && secondResponse != "")
            {
                String[] resolutionLinks = secondResponse.Split('\n');
                if (resolutionLinks.Length >= 7)
                {
                    if (secondResponse.IndexOf("http") >= 0)
                    {
